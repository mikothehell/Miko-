...
local function GetNearbyLootPrompt(pos, maxDist) 
    ...
    for _, v in ipairs(Workspace:GetChildren()) do 
        if v:IsA("ProximityPrompt") and v.Parent and v.Parent:IsA("BasePart") then 
            if (v.Parent.Position - pos).Magnitude <= maxDist then return v end 
        elseif v:IsA("Model") or v:IsA("Folder") then 
            for _, child in ipairs(v:GetChildren()) do 
                if child:IsA("ProximityPrompt") and child.Parent and child.Parent:IsA("BasePart") then 
                    if (child.Parent.Position - pos).Magnitude <= maxDist then return child end 
                end 
            end 
        end 
    end 
...
local function IsEnemy(model) 
    ...
    if string.find(nameLower, "shop") or string.find(nameLower, "display") or string.find(nameLower, "showcase") or string.find(nameLower, "lobby") or string.find(nameLower, "store") or string.find(nameLower, "building") or string.find(nameLower, "glass") or string.find(nameLower, "npc") then return false end 
    return true 
end 
...
local function GetNearestEnemy() 
    ...
    for _, v in ipairs(Workspace:GetChildren()) do 
        ...
        if v:IsA("Folder") or (v:IsA("Model") and not IsEnemy(v)) then 
            for _, child in ipairs(v:GetChildren()) do CheckModel(child) end 
        end 
    end 
    return nearest 
end 
...
CombatTab:CreateToggle({ 
    ...
    Callback = function(Value) 
        autoFarm = Value 
        if not Value then currentTarget = nil isCollectingLoot = false end 
    end, 
}) 
...
CombatTab:CreateDropdown({ 
    ...
    Callback = function(Option) 
        if type(Option) == "table" then farmPosition = Option[1] else farmPosition = Option end 
    end, 
}) 
...
CombatTab:CreateToggle({ 
    ...
    CurrentValue = false, 
    Flag = "AutoLootToggle", 
    Callback = function(Value) autoLoot = Value end, 
}) 
...
CombatTab:CreateToggle({ Name = "Auto Click Chuột Trái (LMB)", CurrentValue = false, Flag = "LmbToggle", Callback = function(Value) useLMB = Value end }) 
CombatTab:CreateToggle({ Name = "Auto Phím [ E ]", CurrentValue = false, Flag = "KeyEToggle", Callback = function(Value) useKeyE = Value end }) 
CombatTab:CreateToggle({ Name = "Auto Phím [ Q ]", CurrentValue = false, Flag = "KeyQToggle", Callback = function(Value) useKeyQ = Value end }) 
CombatTab:CreateToggle({ Name = "Auto Phím [ R ]", CurrentValue = false, Flag = "KeyRToggle", Callback = function(Value) useKeyR = Value end }) 
CombatTab:CreateToggle({ Name = "Auto Phím [ F ]", CurrentValue = false, Flag = "KeyFToggle", Callback = function(Value) useKeyF = Value end }) 

CombatTab:CreateSection("3. Hitbox Phóng To (Max 100)") 

CombatTab:CreateToggle({ Name = "Phóng To Hitbox Quái", CurrentValue = false, Flag = "HitboxToggle", Callback = function(Value) expandHitbox = Value end }) 
...
task.spawn(function() 
    while task.wait(0.15) do 
        if autoFarm and not isCollectingLoot then 
            pcall(function() 
                local isTargetAlive = currentTarget and currentTarget.Parent and currentTarget.Parent:FindFirstChildOfClass("Humanoid") and currentTarget.Parent:FindFirstChildOfClass("Humanoid").Health > 0 
            ...
            pcall(function() 
                ...
                for _, v in ipairs(Workspace:GetChildren()) do 
                    ...
                    if v:IsA("Folder") then 
                        for _, child in ipairs(v:GetChildren()) do ApplyHitbox(child) end 
                    end 
                end 
            end) 
        end 
